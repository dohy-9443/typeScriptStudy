...
let guest :(number | string | boolean) = 123;
guest = '123';
guest = true;

// 문자 혹은 숫자가 가능한 array 타입 지정은?
let guests :(number | string)[] = [1,'2',3];

// 문자 혹은 숫자가 가능한 object 타입 지정은?
let guestObj : { a : (number | string) } = { a : '2' };
...
let myUser :string = 'kim'
let myAge :undefined | number = undefined;
let myMarried :boolean = false;
let me :(string | (undefined | number) | boolean)[] = [myUser, myAge, myMarried];
