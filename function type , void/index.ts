...
function fun2(y) :number {
  return y * 2
}

// 파라미터랑 return값에 type 지정
function fun3(z :number) :number {
  return z * 2
}
...
function q2(x :number | string) :number {
  if (typeof x === 'number') {
    return `${x}`.length
  } else {
    return x.length
...
function q3(pay :number , isHome :boolean , charm :string) {
  let total = 0;
